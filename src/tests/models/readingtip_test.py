...
class TestReadingTip(unittest.TestCase):
    def setUp(self):
        self.owner = User("maija", "jahph5Ie")

    def test_constructor_sets_fields_correctly(self):
        readingtip = ReadingTip("Hyvä kirja", "https://kirjakauppa.fi/123", self.owner)
    ...
    def test_constructor_adds_http(self):
        readingtip = ReadingTip("Keskinkertainen kirja", "kirjakauppa.fi/124", self.owner)
        self.assertEqual(readingtip.link, "http://kirjakauppa.fi/124")

    def test_constructor_preserves_http(self):
        readingtip = ReadingTip("Huono kirja", "http://kirjakauppa.fi/125", self.owner)
